  ...
  protected async analyzeRepo(): Promise<Repo> {
    try {
      ...
      if (!stderr) {
        ...
        const { stdout: repoName } = await execP(
          `echo ${repo.repoUrl}| rev | cut -d'/' -f 1 | rev | cut -d'.' -f 1`
        );
        ...
        const { stdout: authorsString } = await execP(
          `git shortlog -sne < ${tty} | awk '{$1=""; sub(" ", ""); print}' | awk -F'<' '!x[$1]++' | awk -F'<' '!x[$2]++'`
        );
        repo.authors = authorsString.split("\n").slice(0, -1);
        const { stdout: createdDate } = await execP(
          `git log --reverse  --pretty=format:"%ad" | head -n 1`
        );
        repo.createdDate = new Date(createdDate);
        const { stdout: commitCount } = await execP(
          `git log $MERGES_ARG --oneline $commit | wc -l | tr -d ' '`
        );
        repo.commitCount = parseInt(commitCount);
        const { stdout: activeDays } = await execP(
          `git log --pretty=format:"%ad" --date=format:'%Y-%m-%d' | uniq | wc -l`
        );

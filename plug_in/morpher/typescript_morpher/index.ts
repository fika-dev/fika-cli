...
export class TypescriptMorpher extends Morpher {
  ...
  protected findFikaNodes(): void {
    ...
    this._tsNodes = sourceFiles
      .map(sc => {
        const statements = sc.getStatementsWithComments();
        const nodes = statements
          .filter(statement => statement.getLeadingCommentRanges().length > 0)
  ...
  public getArguementsFromAF(iNode: INode): string[] {
    ...
    if (isVS(node)) {
      ...
      return cd
        .getProperties()
        .map(property => `${property.getName()}: ${property.getType().getText()}`);
    } else {
  ...
  addCommentedDecorator() {
    const sc = this._project.addSourceFileAtPath(
      "/Users/wonmojung/M1/fika/tool/src/utils/__test__/comment-decorator.test.data.ts"
    );
    const statements = sc.getStatementsWithComments();
    const [fikaNode] = statements
      .filter(sc => sc.getLeadingCommentRanges().length > 0)
  ...
  getNodeWithFika(node: Node): Node | false {
    ...
    const commentsWithFikaDecorator = comments
      .map(comment => comment.getText())
      .filter(text => text.includes("@Fika("));
